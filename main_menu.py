...
class Frame(tk.Frame):
    def __init__(self, root = None):
        ...
        self.grid_columnconfigure(0, weight=1) # Displays
    ...
    def display_content(self):
        ...
        # Placeholder if tables are empty
            ...
            placeholder_label = tk.Label(wrapper_frame, text = placeholder_text, 
                                         bg = "#BEBEBE", fg = "#000000",
                                         font = ("Averia Serif Libre", 14), justify = tk.CENTER)
            placeholder_label.grid(row = 0, column = 0, padx = 30, pady = 50, sticky = "nsew")
        else:
            welcome_label = tk.Label(wrapper_frame, text = "WELCOME!\nWHAT'S THE PLAN FOR TODAY?", bg = "#BEBEBE",
                     fg = "#000000", font = ("Averia Serif Libre", 14, "bold"))
            welcome_label.grid(row = 0, column = 0, padx = 10, pady = (10,5),
                               sticky = "w")
            current_row = 1

            # Budget Frame
            for i, b in enumerate(self.fetched_budgets):
                frame_budget1 = tk.Frame(wrapper_frame, bg = "#0E5200", bd = 0, width = 250, height = 80)
                frame_budget1.grid(row = current_row, column = 0, padx = 10, pady = 10, sticky = "ew")
                frame_budget1.grid_propagate(False)
                frame_budget1.grid_columnconfigure(0, weight = 1)

                # b[1] is name, b[2] is amount
                tk.Label(frame_budget1, text = f"Budget: {b[1]}", bg = "#0E5200",
                         fg="#FFFFFF", font = ("Averia Serif Libre", 12)).grid(row=0, column=0, pady = (10, 0))
                tk.Label(frame_budget1, text = f"${b[2]}", bg = "#0E5200", 
                         fg="#FFFFFF", font = ("Averia Serif Libre", 12)).grid(row=1, column=0, pady = (10, 0))
            ...
            for j, g in enumerate(self.fetched_goals):
                frame_goal1 = tk.Frame(wrapper_frame, bg = "#0E5200", bd = 0, width = 250, height = 80)
                frame_goal1.grid(row = current_row, column = 0, padx = 10, pady = 10, sticky = "ew")
                frame_goal1.grid_propagate(False)
                frame_goal1.grid_columnconfigure(0, weight = 1)

                # g[1] is name, g[2] is amount
                tk.Label(frame_goal1, text = f"Goal: {g[1]}", bg = "#0E5200",
                         fg="#FFFFFF", font = ("Averia Serif Libre", 12)).grid(row=0, column=0, pady = (10, 0))
                tk.Label(frame_goal1, text = f"Target: ${g[2]}", bg = "#0E5200",
                         fg="#FFFFFF", font = ("Averia Serif Libre", 12)).grid(row=1, column=0, pady = (10, 0))
    ...
    def option_buttons(self):
        ...
        self.quit_button(buttons_frame).grid(row = 3, column = 0, padx = 10, pady = 30, sticky = "ew")

    def create_budget_button(self, parent_frame):
        return tk.Button(parent_frame, text = "Create/Manage Budget", font = ("Averia Serif Libre", 16, "bold"), 
                         bg = "#6FFF6F", cursor = "hand2", fg = "#000000", activebackground = "#6FFF6F", 
                                  command = lambda: print("This goes to Budget Screen."))
    
    def manage_goal_button(self, parent_frame):
        return tk.Button(parent_frame, text = "Add/Manage Goal", font = ("Averia Serif Libre", 16, "bold"), 
                         bg = "#000000", cursor = "hand2", fg = "#6FFF6F", 
                         activebackground = "#000000", activeforeground = "#6FFF6F", 
                         command = self.to_goal_menu)
    
    def quit_button(self, parent_frame):
        return tk.Button(parent_frame, text = "Quit Program", font = ("Averia Serif Libre", 16, "bold"),
                         bg = "#BEBEBE", cursor = "hand2", fg = "#000000", activebackground = "#BEBEBE",
